...
class DOTA2GSI {
    constructor() {
        ...
        this.digest = (rawGSI) => {
            ...
            const players = rawPlayers.map(data => utils_1.parsePlayer(data.player, data.id, rawGSI, this.players));
            const gsi = {
                provider: rawGSI.provider,
                map: utils_1.parseMap(rawGSI.map, this.teams),
                players,
                player: players.find(player => { var _a; return (_a = player.hero) === null || _a === void 0 ? void 0 : _a.selected_unit; }) || null,
                buildings: rawBuildings.map(entry => utils_1.parseBuilding(entry.id, entry.building)),
                draft: {
                    ...
                    radiant: {
                        ...
                        picks: utils_1.parseDraft(rawGSI.draft.team2)
                    },
                    dire: {
                        home_team: rawGSI.draft.team3.home_team,
                        bonus_time: rawGSI.draft.dire_bonus_time,
                        picks: utils_1.parseDraft(rawGSI.draft.team3)
                    }
                }
            };
            this.emit('data', gsi);
