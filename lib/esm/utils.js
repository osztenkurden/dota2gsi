...
export const parseRunes = (minimap) => {
    if (!minimap)
        return { available: [] };
    ...
    for (const [nowTeam, lastTeam] of teams) {
        for (const [tierNow, tierThen] of [
            ...
            if (tierThen.completion_time) {
                ...
                console.log('EXISTING COMPLETION TIME');
                console.log({ result });
            }
            else if (!checkItemTier(tierThen) && checkItemTier(tierNow)) {
                tierNow.completion_time = currentTime;
                console.log('CURRENT COMPLETION TIME');
                console.log({ result });
            }
            else {
                console.log({ then: checkItemTier(tierThen), now: checkItemTier(tierNow) });
            }
        }
    }
    return neutralItems;
